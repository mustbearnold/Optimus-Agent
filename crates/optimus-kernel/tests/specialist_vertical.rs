//! Phase-3 multi-agent vertical: workspace_writer + write_file_handoff.
...
use optimus_kernel::{
    ...
    open_seeded_workflow_registry, run_write_file_handoff, vertical_workspace,
    workspace_writer_descriptor, write_file_handoff_workflow, AgentResultKind, ArtifactStore,
    WorkflowTerminalKind, WriteFileHandoffRequest, WORKSPACE_WRITER_ID, WORKSPACE_WRITER_VERSION,
...
fn seeds_builtin_specialist_and_workflow_registries() {
    ...
    assert_eq!(listed_agents[0].id.as_str(), WORKSPACE_WRITER_ID);
    assert_eq!(listed_agents[0].version.as_str(), WORKSPACE_WRITER_VERSION);
    assert_eq!(
        listed_agents[0],
    ...
    assert_eq!(listed_workflows.len(), 1);
    assert_eq!(listed_workflows[0].id.as_str(), WRITE_FILE_HANDOFF_WORKFLOW_ID);
    assert_eq!(
        listed_workflows[0].version.as_str(),
    ...
    assert_eq!(workflows.list().unwrap().len(), 1);
}
...
fn cancel_request_fences_running_invocation_from_late_success() {
    let dir = tempdir().unwrap();
    let home = dir.path();
    // Pre-create registries and a running invocation path via a first success,
    // then prove cancel API is durable against a fresh invocation id setup.
    // Direct cancel before settle: open registries, begin via a failed path is hard;
    // instead run success then cancel on completed returns false.
    let report = run_write_file_handoff(
        home,
        WriteFileHandoffRequest {
