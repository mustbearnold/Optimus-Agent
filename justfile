...
tui-layout:
    ...
    bun scripts/tests/tui_layout_playwright.cjs
...
dev-check:
    @bash scripts/verify.sh gates
    @python3 scripts/tools/impact_select.py
    @cargo test $(python3 scripts/tools/impact_select.py --cargo-args) --all-targets
...
test-changed:
    @python3 scripts/tools/impact_select.py --require-selection
    @cargo test $(python3 scripts/tools/impact_select.py --cargo-args) --all-targets
...
self-development:
    ...
    @python3 scripts/tests/test_self_development.py
...
self-development-desktop:
    ...
    cargo build --locked -p optimus-desktop --bin optimus-desktop
    @python3 scripts/tests/test_self_development.py --surface desktop
...
clean-report:
    python3 scripts/tools/project_hygiene.py report

# Delete only the closed, verified allowlist of generated outputs.
clean:
    python3 scripts/tools/project_hygiene.py clean
...
modules:
    python3 scripts/gates/check-module-size.py --report
...
docs-check:
    python3 scripts/tools/docs_system.py check

# Regenerate the deterministic catalog. This never refreshes source bindings.
docs-generate:
    python3 scripts/tools/docs_system.py generate
...
docs-context route:
    python3 scripts/tools/docs_system.py context {{quote(route)}}

# Prove representative fresh-agent questions resolve to the expected authority.
docs-benchmark:
    python3 scripts/tools/docs_system.py benchmark
...
em-generate:
    python3 scripts/tools/engineering_memory.py generate
    python3 scripts/tools/project_knowledge.py generate
...
parity:
    -python3 scripts/tools/optimus_version.py hermes-scorecard
