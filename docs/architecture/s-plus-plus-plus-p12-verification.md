...
| S4 Isolation → envelope | `WorkIsolationMode::command_fs_envelope`; Kernel + desktop `open_runtime` load settings |
| S5 Nested breakout hold | `crates/optimus-runtime/tests/cancellation.rs` (setsid escapee + nested systemd scope) |
...
cargo test -p optimus-desktop settings_get_set
```
