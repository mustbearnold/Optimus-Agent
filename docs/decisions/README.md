...
| 0075 | A block owns its lifecycle; rows only paint it | Accepted | The terminal becomes a structured workbench of typed blocks keyed by domain identity (`call_id`, `run_id`), with rendered rows demoted to a draw-time projection: one animation clock replaces the unconditional 40 ms draw, `TurnUpdate` is formalized as the event-adapter seam, block kinds without a real event source are recorded as planned rather than scaffolded, and the phase map splits session.rs before adding to it. |
