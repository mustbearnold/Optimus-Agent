...
class NativeGateTest(unittest.TestCase):
    ...
    def test_hermes_scorecard_is_informational_and_never_blocks(self) -> None:
        result = subprocess.run(
            ["python3", str(versioning.ROOT / "scripts" / "tools" / "optimus_version.py"), "hermes-scorecard"],
            capture_output=True,
            text=True,
            check=False,
        )
    ...
    def test_native_gate_names_every_bar(self) -> None:
        result = subprocess.run(
            ["python3", str(versioning.ROOT / "scripts" / "tools" / "optimus_version.py"), "gate"],
            capture_output=True,
            text=True,
            check=False,
        )
    ...
    def test_foreign_fingerprint_blocks_release_claims(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            scripts.mkdir()
            real = versioning.ROOT / "scripts" / "tools" / "perf_harness.py"
