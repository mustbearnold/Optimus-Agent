...
describe('ProviderKeysPanel', () => {
  it('offers a DeepSeek key field and tells the user no key is configured', async () => {
    const panel = render(
      <ProviderKeysPanel transport={transportWith(() => ({ providers: [absent] }))} active />
    );
  ...
  it('sends a pasted key to the host and clears it from the field', async () => {
    ...
    const transport = transportWith((method, params) => {
    ...
    const panel = render(<ProviderKeysPanel transport={transport} active />);
  ...
  it('refuses to send an empty key', async () => {
    const calls: string[] = [];
    const transport = transportWith((method) => {
      calls.push(method);
      return { providers: [absent] };
    });
    const panel = render(<ProviderKeysPanel transport={transport} active />);
  ...
  it('reports an environment key as in use and offers to replace it', async () => {
    ...
    const panel = render(
      <ProviderKeysPanel transport={transportWith(() => ({ providers: [fromEnv] }))} active />
    );
